...
// - heapKeys are choosen randomly to ensure balance with high probability
struct treap{
  int key;
  int heapKey;
...
/* or INT_MIN if there no such element */
...
void treapInsert(struct treap **parent, int key){
  ...
  for(int dir = LEFT; dir <=RIGHT; dir++){
    if((*parent)->child[dir] != 0 &&
       (*parent)->child[dir]->heapKey > (*parent)->heapKey)
...
void treapDelete(struct treap **parent, int key){
  ...
  if(*parent == 0)
    ...
    for(;;){
      ...
      for(int dir = LEFT; dir <= RIGHT; dir++){
	if((*parent)->child[dir] == 0){
	  /* yes free this node, and promote another kid */
	  ...
	  *parent = toDelete->child[!dir];
      ...
      /* no missing childs, have to rotate down */
      int biggerKid;
