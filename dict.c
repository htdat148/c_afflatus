...
/* dictionary initialization code used in both DictCrete and grow*/
...
static void grow(Dict d){
  Dict d2;			/* new dictionary we will create */
...
/* return the most recently inserted value associated with key */
/* or 0 if no matchinf key is present */

const char * DictSearch(Dict d, const char *key){
  struct elt *e;

  for(e = d->table[hash_function(key)], e != NULL; e=e->next){
    if(!strcmp(e->key, key))
      /* got it */
      return e->value;
  }
...
/* if there is no such record hash no effect */
