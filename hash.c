...
typedef struct hash_node_t{
  ...
  const char *key;		/* key for hash look up */
  struct hash_node_t *next;	/* next node in hash chain */
}hash_node_t;

/* 
 * hash() - Hash fucntion returns a hash number for a given key
 * tptr: pointer to a hash table 
 * key: the key to create the hash number for
 */
...
/* 
 * hash_init() - Initilize a new hash table
 * tptr: pointer to the hash table to Initilize
 * buckets: the number of initial bucket to crate
 *
 */
void hash_init(hash_t *tptr, int buckets){
  /* make sure we allocte something */
  if (buckets == 0)
    buckets=16;
  ...
  /* enssure the bucket is a power of 2 */
...
int hash_lookup(const hash_t *tptr, const char *key){
  ...
  /* find the entry in the hash tabale */
...
int hash_insert(hash_t *tptr, const char *key, int data){
  ...
  /* check to see the entry exists */
  if((tmp = hash_lookup(tptr, key)) != HASH_FAIL)
    return tmp;
  
  /* expand the table if need */
  while(tptr->entries >= HASH_LIMIT*tptr->size)
    rebuild_table(tptr);

  /* inseart the new entry */
  h=hash(tptr, key);
...
int hash_delete(hash_t *tptr, const char *key){
  ...
  /* if node is the head of bucket, we have it easy */
  if(node == tptr->bucket[h]){
    tptr->bucket[h] = node->next;
  }
  else{
    /* find the node before the node we remove */
...
/* 
 * inthash_entries() - Delete the entrie table, and all remaining
 * entries
 */
int inthash_entries(hash_t *tptr){
  return tptr->entries;
}

/* 
 * hash_destroy() - Delete entire table, and all remaining entries
 * 
*/
...
/* 
 * hash_stats() - return a string with stats about
 * a hash table
 */
